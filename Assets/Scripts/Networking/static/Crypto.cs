...
namespace TBD.Crypto {
  ...
  public static class JWToken {
    ...
    public static string CreateToken(string username, byte[] secret) {
    ...
    public static JObject DecodeToken(string token) {
      try {
				...
				string sTokenHeader = Base64UrlDecode(token.Split('.')[0]);
				string sTokenPayload = Base64UrlDecode(token.Split('.')[1]);
				byte[] sTokenHash = Encoding.UTF8.GetBytes(Base64UrlDecode(token.Split('.')[2]));
