...
namespace TBD.Crypto {
  ...
  public static class JWToken {
    ...
    public static byte[] secretKey = new byte[512];
    ...
    public static string CreateToken(string username) {
      ...
      var dPayload = new Dictionary<string, object> {
        ...
        { "clearance", Io.GetClearance(username) }
      };


      string sHeaderB64 = Base64UrlEncode(JsonConvert.SerializeObject(dHeader));
      string sPayloadB64 = Base64UrlEncode(JsonConvert.SerializeObject(dPayload));

      var sha512 = new HMACSHA512(secretKey);
      byte[] sha512Hash = sha512.ComputeHash(Encoding.UTF8.GetBytes(dHeader +"."+ dPayload));
    ...
    public static bool ValidateToken(string token) {
      return ((int)DecodeToken(token)["state"] == (int)JWTErrorCode.Valid);
    }
    ...
    public static JObject DecodeToken(string token) {
      try {
        ...
        JObject sTokenHeader = token.Split('.')[0].toJObject();
        JObject sTokenPayload = token.Split('.')[1].toJObject();
        JObject sTokenHash = token.Split('.')[2].toJObject();

        if (sTokenHeader == null || sTokenPayload == null || sTokenHash == null)
          throw new FormatException("Token is in invalid format!");
        
        
        return sTokenPayload;
      } catch {
